...
app.post('/api/chat-heartbeat', (req, res) => {
  ...
  if (status === 'terminated') {
    console.log(`Chat fetcher terminated for video ${videoId}. Stats:`, stats);
  }
...
function startMonitoring() {
  ...
  if (state.videoId && !chatProcess) {
    ...
    const fs = require('fs');
    const finalScriptPath = fs.existsSync(scriptPath) ? scriptPath : './python/chat_fetcher.py';

    console.log(`Using chat fetcher: ${finalScriptPath}`);
